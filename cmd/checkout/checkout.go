...
func checkoutMain(opts *CheckoutOpts, version string) error {
	...
	isStewRepo, err := utils.PathExists(filepath.Join(cwd, ".stew"))
	if err != nil {
		msg := "fatal, failed to access stew repository"
		fmt.Println(msg)
	...
	url := fmt.Sprintf("%s/api/v1/projects/%s/pins/vs", cfg.Remote.Server, cfg.Remote.Project, opts.Version)
